...
function getIsoline(coordinates, objectID, feature) {
    ...
    if (places_aoi_street_distance.features.length === places_aoi.features.length) {
        ...
        svg.selectAll(".village").data(places_aoi_street_distance.features).attr("data-distance_to_street", function(d) {
            return d.properties.connections.distance_to_street;
        }).attr("data-touches_street", function(d) {
        ...
        svg.selectAll(".village-group").each(function(d) {
            ...
            current_el.append("text").text(d.properties.name).style("font-weight", "bold").attr("y", "30");
            current_el.append("text").text("Distance to Street: " + Math.round(d.properties.connections.distance_to_street) + " meter").attr("y", "45");
        });
        update(0);
    }
}
